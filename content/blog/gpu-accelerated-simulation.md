...
## The SIR: What the GPU Actually Consumes

The previous section mentioned that SharedCodegen generates code from the SIR. But what is the SIR, and why does it exist as a separate representation?

skalp has four intermediate representations — HIR for intent, MIR for cycle-accurate RTL, LIR for technology primitives, and SIR for simulation execution. The [full IR pipeline](/blog/skalp-ir-pipeline/) covers all four in depth. Here we focus on what makes SIR the right shape for GPU dispatch.

**Why not generate shaders directly from MIR?** MIR is the cycle-accurate RTL representation — it has `always_ff` and `always_comb` processes, hierarchical module instances, clock domain specifications, and sensitivity lists. It's what skalp uses to generate SystemVerilog, and it's an accurate model of hardware behavior. But it's the wrong shape for a GPU. Module hierarchy means the code generator would need to inline instances recursively. Processes mix combinational and sequential logic in the same block (an `always_ff` block has both the flip-flop and its input logic). Sensitivity lists require dynamic scheduling — evaluating only the blocks whose inputs changed. And MIR preserves the original signal names and scoping, which means name collisions after flattening.

SIR solves all of these problems by transforming the design into a flat, pre-scheduled, GPU-friendly representation:

- **Flat**: All hierarchy is elaborated and flattened. A module that instantiates a FIFO that instantiates a memory doesn't have three levels of scope — it has one flat list of nodes with prefixed signal names.
- **Separated**: Combinational nodes and sequential nodes are in separate lists. The GPU evaluates them in different kernels with different buffer bindings.
- **Pre-sorted**: Combinational nodes are topologically sorted at SIR construction time. Every simulation step walks the sorted list in order — no dynamic scheduling, no dependency checking at runtime.
- **Named**: A `NameRegistry` maps hierarchical paths to flat signal names, so the testbench can refer to `fifo.mem[3]` and the runtime resolves it to the correct buffer offset.

### SirModule

The `SirModule` struct is what SharedCodegen receives:
...
- `inputs` / `outputs`: Flat port declarations with width and type information. Each `SirPort` carries a `SirType` (which maps to Metal/C++ types through `TypeMapper`), an optional clock domain, and a source span for error reporting.

- `signals`: Internal signals — the wires between nodes. Each signal knows its `driver_node` (which node produces its value) and `fanout_nodes` (which nodes consume it). This explicit fanout information is what makes topological sorting possible.

- `combinational_nodes` / `sequential_nodes`: Two separate node lists, by design. The combinational kernel evaluates `combinational_nodes` in topological order. The sequential kernel evaluates `sequential_nodes` on clock edges. Putting them in separate lists means the code generator doesn't need to filter or classify at generation time — it just iterates.

- `state_elements`: A map from signal name to `StateElement`, which carries reset value, clock, and reset signal information. This drives the `Registers` struct generation and the double-buffer snapshot logic.

- `sorted_combinational_node_ids`: The pre-computed topological order. This is the evaluation order that the combinational kernel follows. Once computed at SIR construction time, it never changes — every simulation step uses the same order.

- `name_registry`: Maps hierarchical names (like `uart_tx.shift_reg`) to flat internal names (like `uart_tx__shift_reg`). The testbench calls `set_input("uart_tx.shift_reg", &value)`, and the runtime uses the registry to find the correct buffer offset.

### SirNode and SirNodeKind

Each node represents a single operation in the dataflow graph:
...
A `SignalRef` is a signal ID with an optional bit range — so a node can read bits `[7:4]` of a signal without a separate slice node (though explicit `Slice` nodes exist for complex cases).

The `SirNodeKind` enum covers every operation the GPU needs to evaluate:

```rust
pub enum SirNodeKind {
    // Arithmetic and logic
    BinaryOp(BinaryOperation),  // Add, Sub, Mul, And, Or, Xor, shifts, comparisons
    UnaryOp(UnaryOperation),    // Not, Neg, reduction (RedAnd, RedOr, RedXor)

    // Multiplexing
    Mux,                                  // 2:1 mux (condition, true, false → output)
    ParallelMux { num_cases, match_values, result_width },  // N:1 priority mux (match/case)

    // Bit manipulation
    Concat,                              // Concatenate inputs into a wider signal
    Slice { start, end },                // Extract a bit range

    // Constants and references
    Constant { value, width },           // Literal value
    SignalRef { signal },                // Direct signal reference

    // Sequential elements
    FlipFlop { clock_edge },             // D flip-flop with edge sensitivity
    Latch { enable },                    // Level-sensitive latch
    Memory { depth, width },             // Memory array

    // Array operations
    ArrayRead,                           // Dynamic array index read
    ArrayWrite,                          // Dynamic array index write
...
Combinational nodes are `BinaryOp`, `UnaryOp`, `Mux`, `ParallelMux`, `Concat`, `Slice`, `Constant`, `SignalRef`, `ArrayRead`. Sequential nodes are `FlipFlop`, `Latch`, `Memory`, `ArrayWrite`. The separation is structural — a `FlipFlop` node is always in `sequential_nodes`, a `BinaryOp` is always in `combinational_nodes`. The code generator doesn't need to inspect `SirNodeKind` to decide which kernel a node belongs to.

### Topological Sorting

The topological sort is the single most important operation in SIR construction. If node B reads signal S and node A writes signal S, then A must evaluate before B. Get this wrong and the simulation reads stale values — the output of A from the previous cycle instead of the current one.

SIR uses Kahn's algorithm in `finalize_topological_order()`. The algorithm:

```
1. Build an in-degree map: for each combinational node, count how many
   of its input signals are produced by other combinational nodes.

2. Initialize a queue with all nodes that have in-degree 0 — these are
   nodes whose inputs come entirely from primary inputs, registers, or
   constants. They can evaluate immediately.

3. While the queue is not empty:
   a. Dequeue a node, add it to the sorted order.
   b. For each signal this node produces, find all nodes that consume
      that signal (using the fanout list).
   c. Decrement those consumers' in-degree by 1.
   d. If any consumer's in-degree reaches 0, enqueue it.

4. If sorted order contains all combinational nodes, done.
   If not, there's a combinational cycle — log a warning and append
   the remaining nodes in their original order as a fallback.
```

The algorithm runs once, at SIR construction time. The result is stored in `sorted_combinational_node_ids`. Every simulation step — whether single-step, batched, GPU, or CPU — walks this list in order. No priority queues, no event scheduling, no dependency checking. Just a flat array of node IDs, evaluated left to right.

Combinational cycles (A drives B drives A) are a design error, but they happen in practice — especially during incremental development when a feedback path hasn't been broken with a register yet. The fallback (appending unsorted nodes) means the simulation still runs, though the results for signals in the cycle may not converge. The warning message tells the designer which signals form the cycle, so they can fix the design.

The cost of pre-computing the order is O(N + E) where N is the number of combinational nodes and E is the number of signal edges. For a 10K-node design, this takes microseconds — negligible compared to the simulation itself.

### MIR → SIR Conversion

The `convert_mir_to_sir_with_hierarchy()` function is the entry point for SIR construction. It takes the full MIR design (all modules) and a top-level module, and produces a flat `SirModule`:
...
│  ├─ instance A  │            │  ├─ sequential_nodes     │
│  │   └─ ...     │            │  ├─ state_elements       │
│  └─ instance B  │            │  ├─ sorted_node_ids      │
│      └─ ...     │            │  └─ name_registry        │
└─────────────────┘            └──────────────────────────┘
```

The conversion runs in phases:

1. **Port conversion**: Top-level MIR ports become SIR ports. Names are registered in the `NameRegistry`.

2. **Signal conversion**: MIR signals become SIR signals. Each signal is checked for sequential behavior (assigned in `always_ff` blocks) and marked with `is_state`.

3. **Variable conversion**: MIR variables (from `let` bindings) become additional SIR signals.

4. **Instance flattening**: For each module instance, the converter recursively elaborates the child module. All child ports, signals, and logic are prefixed with the instance path (e.g., `fifo.wr_ptr`) and added to the flat SIR module. Circular instantiation is detected and reported as an error.

5. **Logic conversion**: MIR processes are classified as combinational or sequential and converted to SIR nodes. Combinational blocks become trees of `BinaryOp`, `UnaryOp`, `Mux`, and `Concat` nodes. Sequential blocks produce `FlipFlop` nodes with the appropriate clock edge. `if`/`case` statements become `Mux`/`ParallelMux` chains.

6. **Clock domain extraction**: Clock signals are identified from the MIR's clock domain declarations and from sequential node sensitivity lists.

7. **Pipeline register insertion**: If the module has a `pipeline_config`, pipeline registers are automatically inserted between stages.

8. **Topological ordering**: `finalize_topological_order()` runs Kahn's algorithm to produce `sorted_combinational_node_ids`.

The converter maintains a `signal_map` that tracks the correspondence between MIR signal names and SIR internal names. When flattening instances, this map is extended with prefixed names — `fifo.wr_ptr` in the MIR becomes `fifo__wr_ptr` in the SIR (double underscore to avoid collisions with signals that legitimately contain single underscores). The `NameRegistry` records both the hierarchical path and the flat name, so the testbench can use either form.

The key design decision: MIR→SIR for behavioral simulation, not LIR→SIR. MIR preserves multi-bit semantics — a 32-bit add is one `BinaryOp::Add` node, not 32 full-adder primitives. This maps naturally to GPU types: a 32-bit add becomes `uint a + uint b` in the Metal shader, a single ALU instruction. If SIR were built from LIR (which decomposes to gate-level primitives), the same add would be hundreds of gate evaluations — dramatically less efficient on a GPU. LIR→SIR is used only for gate-level fault simulation, where the per-gate granularity is the whole point.
