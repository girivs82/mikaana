...
Traditionally, boundary-only NCL is considered **quasi-delay-insensitive** — it assumes combinational logic settles before the next wavefront, a weaker guarantee than full NCL. skalp eliminates this assumption. After placement and routing, async STA computes the worst-case delay through every combinational path within each stage. Where paths fork with different delays, the tool inserts matched delays on the ready signals of faster paths, *enforcing* that the completion detector can't fire until the slowest path has settled. The timing isn't assumed — it's measured and guaranteed by construction. (More on this in the async STA section below.)
