...
Multi-clock designs need a different testing approach. Instead of `clock()` which advances a single clock, use `clock_multi()` to advance each clock domain independently. This lets you test synchronizer latency, async FIFO behavior under different clock ratios, and full CDC paths.

Here are tests from `tests/ch08_test.rs`:
